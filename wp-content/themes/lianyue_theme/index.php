			...
			query_posts($args);
				...
				?>
					<div class="changeDiv">
						...
						<a href="<?php the_permalink() ?>" title="<?php the_title(); ?>" rel="bookmark"><?php echo wp_image(700,270); ?></a>
					</div>
				<?php
				endwhile;
			wp_reset_query();
		...
		<div class="newest left" id="newest">
			...
			<?php
				...
				while (have_posts()) : the_post();
					...
					$color = post_color();
					$content.= "<li>
								<a href=\"{$link}\" class=\"img\" title=\"{$title}\">{$image}</a>
								<h3><a href=\"{$link}\" title=\"{$title}\"  {$color} class=\"link\" ><em>{$i}</em>{$title}</a></h3>
								</li>";
					$i++;
				endwhile;
				wp_reset_query();
					...
					foreach ($categories as $category) {
						...
						query_posts("showposts=6&cat=$category");
							while (have_posts()) : the_post();
								...
								$home_cat_content.='<li>';
								$home_cat_content.='<a href="'.get_permalink().'" '.post_color().' title="'.get_the_title().'" rel="bookmark">';
					...
					foreach ($categories as $category) {
						...
						query_posts("showposts=8&cat=$category");
							...
							while (have_posts()) : the_post();
								...
								$cat_list.= '<a href="'.get_permalink().'" title="'.get_the_title().'" '.post_color().' rel="bookmark">';
